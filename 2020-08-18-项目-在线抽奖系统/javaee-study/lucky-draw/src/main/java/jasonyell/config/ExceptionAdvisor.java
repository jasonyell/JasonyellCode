...
import jasonyell.exception.BaseException;
...
@Slf4j   // lombok 定义的日志注解，可以使用 log 变量，来记录日志
@ControllerAdvice
public class ExceptionAdvisor {
    ...
    @ResponseStatus(HttpStatus.NOT_FOUND)   // 响应状态码 404 响应体为 null
    ...
    /**
     * SpringMVC 调用Controller方法是，类似这样的处理方式：
     *  try{
     *      controller.方法调用();
     ...
     *  }
     *  日志的使用：
     *      日志的使用级别：trace debug info warn error
     *      根据配置的日志打印级别，调用方法 >= 配置的级别，就会打印
     *
     */

    @ExceptionHandler(BaseException.class)
    @ResponseStatus(HttpStatus.OK)  // 返回响应状态码 200
    @ResponseBody // f返回j son
